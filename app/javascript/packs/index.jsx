...
const App = () => {
  ...
  return (
    <div className='flex h-screen subpixel-antialiased'>
      ...
      <Notes notes={notes} setNotes={setNotes} showNote={showNote} activeNoteId={activeNoteId} setActiveNoteId={setActiveNoteId} isSaving={isSaving} cleanInputFields={cleanInputFields} />
      <div className='editor p-5 w-full'>
        <div className='form-group flex flex-col mb-4 border border-gray-300'>
          <input type='text' type='title' className='form-control px-2 py-3' placeholder='Title...' onChange={(e) => setTitle(e.target.value)} value={title} disabled={isSaving} />
        </div>
        ...
        <div className="tray">
          <span className={`status-text  ml-3 ${isSaving ? 'saving' : 'saved'}`}>
            { !isSaving && <i className='fas fa-check mr-1'></i> }
          </span>
          <div className='action-buttons text-sm ml-auto'>
            <button className='bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded' onClick={handleSubmit} disabled={isSaving}>
